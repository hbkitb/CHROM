codeunit 50149 "50149_Diverse_OP_ERPG"
{
    ...
    begin
        ...
        Svar01 := Dialog.StrMenu('Imp LagKart,Imp DebKart,Imp Medarb,Imp Varegrp,Imp KostVAL,Imp FinKart');

        case Svar01 of
            1:
                begin
                    Message('LagKart');
                    Xmlport.Run(Xmlport::"50151_Imp_LagKart");
                end;
            2:
                begin
                    Message('DebKart');
                    //Xmlport.Run(Xmlport::"50152_Imp_DebKart");
                    cust.Reset;
                    if cust.FindSet then
                        repeat
                            cust."Gen. Bus. Posting Group" := 'INDENLANDS';
                            cust."VAT Bus. Posting Group" := 'INDENLANDS';
                            cust.Modify;
                        until cust.Next = 0;
                    Vend.Reset;
                    if Vend.FindSet then
                        repeat
                            Vend."Gen. Bus. Posting Group" := 'INDENLANDS';
                            Vend."VAT Bus. Posting Group" := 'INDENLANDS';
                            Vend.Modify;
                        until Vend.Next = 0;
            ...
            3:
                begin
                    Message('MedKart');
                    Xmlport.Run(Xmlport::"50153_Imp_MedKart");
                end;
            4:
                begin
                    Message('VareGrp');
                    Xmlport.Run(Xmlport::"50154_Imp_VareBogGrp");
                end;
            5:
                begin
                    Message('KostVAL');
                    Xmlport.Run(Xmlport::"50155_Imp_LagKostVal");
                end;
            6:
                begin
                    Message('FinKart');
                    Xmlport.Run(Xmlport::"50156_Imp_FinKart");
                end;
        end;
        /* 230719 //
        cust.Reset();
